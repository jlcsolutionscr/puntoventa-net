...
namespace LeandroSoftware.ServicioWeb.Servicios
{
    public interface IFacturacionService
    {
        ...
        int ObtenerTotalListaFacturas(int intIdEmpresa, int intIdSucursal, bool bolExcluyeNulos, bool bolFiltraPendientesPago, int intIdFactura, string strNombre, string strIdentificacion, string strFechaFinal);
        IList<FacturaDetalle> ObtenerListadoFacturas(int intIdEmpresa, int intIdSucursal, bool bolExcluyeNulos, bool bolFiltraPendientesPago, int numPagina, int cantRec, int intIdFactura, string strNombre, string strIdentificacion, string strFechaFinal);
    ...
    public class FacturacionService : IFacturacionService
    {
        ...
        public int ObtenerTotalListaFacturas(int intIdEmpresa, int intIdSucursal, bool bolExcluyeNulos, bool bolFiltraPendientesPago, int intIdFactura, string strNombre, string strIdentificacion, string strFechaFinal)
        {
            if (_serviceScopeFactory == null) throw new Exception("Service factory not set");
            using (var dbContext = _serviceScopeFactory.CreateScope().ServiceProvider.GetRequiredService<LeandroContext>())
            {
        ...
        public IList<FacturaDetalle> ObtenerListadoFacturas(int intIdEmpresa, int intIdSucursal, bool bolExcluyeNulos, bool bolFiltraPendientesPago, int numPagina, int cantRec, int intIdFactura, string strNombre, string strIdentificacion, string strFechaFinal)
        {
            if (_serviceScopeFactory == null) throw new Exception("Service factory not set");
            using (var dbContext = _serviceScopeFactory.CreateScope().ServiceProvider.GetRequiredService<LeandroContext>())
            {
                ...
                try
                {
                    ...
                    foreach (var factura in listado)
                    {
                        ...
                        FacturaDetalle item = new FacturaDetalle(factura.IdFactura, factura.ConsecFactura, factura.NombreCliente, factura.Cliente.IdCliente > 1 ? factura.Cliente.Identificacion : "", factura.Fecha.ToString("dd/MM/yyyy"), factura.Gravado, factura.Exonerado, factura.Excento, factura.Impuesto, factura.Total, 0, strEstado, "", factura.Nulo, factura.PendientePago);
        ...
        public string AgregarProforma(Proforma proforma)
        {
            ...
            using (var dbContext = _serviceScopeFactory.CreateScope().ServiceProvider.GetRequiredService<LeandroContext>())
            {
                try
                {
                    ...
                    if (empresa == null) throw new BusinessException("Empresa no registrada en el sistema. Por favor, pongase en contacto con su proveedor del servicio.");
                    if (empresa.TipoContrato < StaticTipoContrato.PlanPYMES) throw new BusinessException("El plan de servicios contratado no permite registrar la transacción. Por favor consulte con su proveedor del servicio.");
        ...
        public void AnularProforma(int intIdProforma, int intIdUsuario, string strMotivoAnulacion)
        {
            ...
            using (var dbContext = _serviceScopeFactory.CreateScope().ServiceProvider.GetRequiredService<LeandroContext>())
            {
                try
                {
                    ...
                    Empresa empresa = dbContext.EmpresaRepository.Find(proforma.IdEmpresa);
                    if (empresa == null) throw new BusinessException("Empresa no registrada en el sistema. Por favor, pongase en contacto con su proveedor del servicio.");
                    if (empresa.TipoContrato < StaticTipoContrato.PlanPYMES) throw new BusinessException("El plan de servicios contratado no permite anular proformas. Por favor consulte con su proveedor del servicio.");
                    proforma.Nulo = true;
            ...
            using (var dbContext = _serviceScopeFactory.CreateScope().ServiceProvider.GetRequiredService<LeandroContext>())
            {
                ...
                try
                {
                    ...
                    foreach (var proforma in proformas)
                    {
                        ...
                        FacturaDetalle item = new FacturaDetalle(proforma.IdProforma, proforma.ConsecProforma, proforma.NombreCliente, proforma.Cliente.IdCliente > 1 ? proforma.Cliente.Identificacion : "", proforma.Fecha.ToString("dd/MM/yyyy"), proforma.Gravado, proforma.Exonerado, proforma.Excento, proforma.Impuesto, proforma.Total, 0, strEstado, "", proforma.Nulo, false);
        ...
        public string AgregarApartado(Apartado apartado)
        {
            ...
            using (var dbContext = _serviceScopeFactory.CreateScope().ServiceProvider.GetRequiredService<LeandroContext>())
            {
                try
                {
                    ...
                    apartado.Procesado = empresa.TipoContrato < StaticTipoContrato.PlanEmpresarial2;
        ...
        public void AnularApartado(int intIdApartado, int intIdUsuario, string strMotivoAnulacion)
        {
            ...
            using (var dbContext = _serviceScopeFactory.CreateScope().ServiceProvider.GetRequiredService<LeandroContext>())
            {
                try
                {
                    ...
                    Empresa empresa = dbContext.EmpresaRepository.Find(apartado.IdEmpresa);
                    if (empresa == null) throw new BusinessException("Empresa no registrada en el sistema. Por favor, pongase en contacto con su proveedor del servicio.");
                    if (empresa.TipoContrato < StaticTipoContrato.PlanEmpresarial2) throw new BusinessException("El plan de servicios contratado no permite anular apartados. Por favor consulte con su proveedor del servicio.");
                    SucursalPorEmpresa sucursal = dbContext.SucursalPorEmpresaRepository.FirstOrDefault(x => x.IdEmpresa == apartado.IdEmpresa && x.IdSucursal == apartado.IdSucursal);
                    if (sucursal == null) throw new BusinessException("Sucursal no registrada en el sistema. Por favor, pongase en contacto con su proveedor del servicio.");
                    if (sucursal.CierreEnEjecucion) throw new BusinessException("Se está ejecutando el cierre en este momento. No es posible registrar la transacción.");
                    apartado.Nulo = true;
        ...
        public IList<FacturaDetalle> ObtenerListadoApartados(int intIdEmpresa, int intIdSucursal, bool bolFiltraEstado, bool bolAplicado, bool bolExcluyeAplicados, bool bolExcluyeNulos, int numPagina, int cantRec, int intIdApartado, string strNombre, string strFechaFinal)
        {
            ...
            using (var dbContext = _serviceScopeFactory.CreateScope().ServiceProvider.GetRequiredService<LeandroContext>())
            {
                ...
                try
                {
                    ...
                    {
                        string strEstado = "Activa";
                        FacturaDetalle item = new FacturaDetalle(apartado.IdApartado, apartado.ConsecApartado, apartado.NombreCliente, apartado.Cliente.IdCliente > 1 ? apartado.Cliente.Identificacion : "", apartado.Fecha.ToString("dd/MM/yyyy"), apartado.Gravado, apartado.Exonerado, apartado.Excento, apartado.Impuesto, apartado.Total, apartado.Total - apartado.MontoAdelanto, strEstado, "", apartado.Nulo, false);
        ...
        public string AgregarOrdenServicio(OrdenServicio ordenServicio)
        {
            ...
            using (var dbContext = _serviceScopeFactory.CreateScope().ServiceProvider.GetRequiredService<LeandroContext>())
            {
                try
                {
                    ...
                    if (empresa == null) throw new BusinessException("Empresa no registrada en el sistema. Por favor, pongase en contacto con su proveedor del servicio.");
                    if (empresa.TipoContrato < StaticTipoContrato.PlanEmpresarial1) throw new BusinessException("El plan de servicios contratado no permite registrar la transacción. Por favor consulte con su proveedor del servicio.");
                    ...
                    if (empresa.Modalidad == StaticModalidadEmpresa.Restaurante)
                    {
                        if (ordenServicio.IdPuntoDeServicio > 0)
                        {
                            ...
                            if (puntoDeServicio != null)
                            {
                                ...
                                dbContext.NotificarModificacion(puntoDeServicio);
                                AgregarTiqueteDespachoOrdenServicio(ordenServicio, ordenServicio.DetalleOrdenServicio, puntoDeServicio.Descripcion, dbContext);
                            }
                        }
                    }
                    dbContext.Commit();
            ...
            using (var dbContext = _serviceScopeFactory.CreateScope().ServiceProvider.GetRequiredService<LeandroContext>())
            {
                try
                {
                    ...
                    if (empresa.Modalidad == StaticModalidadEmpresa.Restaurante)
                    {
                        ...
                        {
                            AgregarTiqueteDespachoOrdenServicio(ordenServicio, nuevoDetalle, puntoDeServicio.Descripcion, dbContext);
                        }
                    }
        ...
        private void AgregarTiqueteDespachoOrdenServicio(OrdenServicio ordenServicio, ICollection<DetalleOrdenServicio> detalleOrdenServicio, string strPuntoServicio, LeandroContext dbContext)
        {
        ...
        public void AnularOrdenServicio(int intIdOrdenServicio, int intIdUsuario, string strMotivoAnulacion)
        {
            ...
            using (var dbContext = _serviceScopeFactory.CreateScope().ServiceProvider.GetRequiredService<LeandroContext>())
            {
                try
                {
                    ...
                    Empresa empresa = dbContext.EmpresaRepository.Find(ordenServicio.IdEmpresa);
                    if (empresa == null) throw new BusinessException("Empresa no registrada en el sistema. Por favor, pongase en contacto con su proveedor del servicio.");
                    if (empresa.TipoContrato < StaticTipoContrato.PlanEmpresarial1) throw new BusinessException("El plan de servicios contratado no permite anular órdenes de servicio. Por favor consulte con su proveedor del servicio.");
                    SucursalPorEmpresa sucursal = dbContext.SucursalPorEmpresaRepository.FirstOrDefault(x => x.IdEmpresa == ordenServicio.IdEmpresa && x.IdSucursal == ordenServicio.IdSucursal);
                    if (sucursal == null) throw new BusinessException("Sucursal no registrada en el sistema. Por favor, pongase en contacto con su proveedor del servicio.");
                    if (sucursal.CierreEnEjecucion) throw new BusinessException("Se está ejecutando el cierre en este momento. No es posible registrar la transacción.");
                    ...
                    if (puntoDeServicio != null)
                    {
                        puntoDeServicio.IdOrden = 0;
                        dbContext.NotificarModificacion(puntoDeServicio);
                        List<TiqueteDespachoMercancia> tiquetes = dbContext.TiqueteDespachoMercanciaRepository.Where(x => x.IdEmpresa == ordenServicio.IdEmpresa && x.IdSucursal == ordenServicio.IdSucursal && x.IdReferencia == ordenServicio.IdOrden).ToList();
                        dbContext.TiqueteDespachoMercanciaRepository.RemoveRange(tiquetes);
                    }
                    dbContext.Commit();
                }
                catch (BusinessException)
        ...
        public IList<FacturaDetalle> ObtenerListadoOrdenServicio(int intIdEmpresa, int intIdSucursal, bool bolFiltraEstado, bool bolAplicado, bool bolExcluyeAplicados, bool bolExcluyeNulos, int numPagina, int cantRec, int intIdOrdenServicio, string strNombre, string strFechaFinal)
        {
            ...
            using (var dbContext = _serviceScopeFactory.CreateScope().ServiceProvider.GetRequiredService<LeandroContext>())
            {
                ...
                try
                {
                    ...
                    {
                        string strEstado = "Activa";
                        FacturaDetalle item = new FacturaDetalle(ordenServicio.IdOrden, ordenServicio.ConsecOrdenServicio, ordenServicio.NombreCliente, ordenServicio.Cliente.IdCliente > 1 ? ordenServicio.Cliente.Identificacion : "", ordenServicio.Fecha.ToString("dd/MM/yyyy"), ordenServicio.Gravado, 0, ordenServicio.Excento, ordenServicio.Impuesto, ordenServicio.Total, ordenServicio.Total - ordenServicio.MontoAdelanto, strEstado, ordenServicio.Descripcion, ordenServicio.Nulo, false);
        ...
        public IList<TiqueteDespachoMercancia> ObtenerListadoTiqueteOrdenServicio(int intIdEmpresa, int intIdSucursal, int intIdOrden, string strCategoria, bool bolFiltrarPendientes, bool bolSortedDesc)
        {
            if (_serviceScopeFactory == null) throw new Exception("Service factory not set");
            using (var dbContext = _serviceScopeFactory.CreateScope().ServiceProvider.GetRequiredService<LeandroContext>())
            {
                try
                {
                    var listado = dbContext.TiqueteDespachoMercanciaRepository.Where(x => x.IdEmpresa == intIdEmpresa && x.IdSucursal == intIdSucursal && x.Impresora != "");
                    if (strCategoria != "")
                        listado = listado.Where(x => x.Impresora == strCategoria);
                    if (intIdOrden > 0)
                        listado = listado.Where(x => x.IdReferencia == intIdOrden);
        ...
        public void ActualizarEstadoTiqueteOrdenServicio(int intIdTiquete, bool bolImpreso)
        {
            ...
            using (var dbContext = _serviceScopeFactory.CreateScope().ServiceProvider.GetRequiredService<LeandroContext>())
            {
                try
                {
                    TiqueteDespachoMercancia tiquete = dbContext.TiqueteDespachoMercanciaRepository.Where(x => x.IdTiquete == intIdTiquete).FirstOrDefault();
        ...
        public string AgregarDevolucionCliente(DevolucionCliente devolucion)
        {
            ...
            using (var dbContext = _serviceScopeFactory.CreateScope().ServiceProvider.GetRequiredService<LeandroContext>())
            {
                ...
                try
                {
                    ...
                    if (!factura.Procesado) throw new BusinessException("La factura asingada a la devolución no ha sido procesada por el cierre y puede ser anulada.");
        ...
        public IList<FacturaDetalle> ObtenerListadoDevolucionesPorCliente(int intIdEmpresa, int intIdSucursal, int numPagina, int cantRec, int intIdDevolucion, string strNombre, string strFechaFinal)
        {
            ...
            using (var dbContext = _serviceScopeFactory.CreateScope().ServiceProvider.GetRequiredService<LeandroContext>())
            {
                ...
                try
                {
                    ...
                    {
                        string strEstado = "Activa";
                        FacturaDetalle item = new FacturaDetalle(devolucion.IdDevolucion, devolucion.IdDevolucion, devolucion.NombreCliente, devolucion.Cliente.Identificacion, devolucion.Fecha.ToString("dd/MM/yyyy"), devolucion.Gravado, 0, devolucion.Excento, devolucion.Impuesto, devolucion.Total, 0, strEstado, "", devolucion.Nulo, false);
            ...
            using (var dbContext = _serviceScopeFactory.CreateScope().ServiceProvider.GetRequiredService<LeandroContext>())
            {
                ...
                try
                {
                    ...
                    if (procesando != null)
                    {
                        ...
                        if (lastProcessAt.AddMinutes(2.00) < fechaActual)
                        {
                            procesando.Valor = fechaActual.ToString("dd-MM-yyyy HH:mm:ss");
                            dbContext.Commit();
                ...
                if (stringBuilder.Length > 0)
                {
                    ...
                    {
                        ["nombre"] = "logErrores-" + fechaActual.ToString("ddMMyyyy-HH-mm-ss") + ".txt",
                    ...
                    {
                        if (_logger != null) _logger.LogError("Error al enviar detalle de errores de procesamiento de documentos pendientes: ", ex);
                    }
                }
            }
        }
        ...
        void ProcesarMensajeReceptor(LeandroContext dbContext, POPEmail correo, bool bolIvaAplicable)
        {
            ...
            SucursalPorEmpresa sucursal = dbContext.SucursalPorEmpresaRepository.FirstOrDefault(x => x.IdEmpresa == empresa.IdEmpresa);
            if (sucursal == null) throw new BusinessException("La empresa no posee sucursales activas. Por favor consulte con su proveedor del servicio.");
            ...
            if (terminal == null) throw new BusinessException("La empresa no posee terminales para la sucursal activas. Por favor, pongase en contacto con su proveedor del servicio.");
            DocumentoElectronico documentoMR = ComprobanteElectronicoService.GeneraMensajeReceptor(strDatos, empresa, dbContext, sucursal.IdSucursal, terminal.IdTerminal, 0, bolIvaAplicable);
            dbContext.DocumentoElectronicoRepository.Add(documentoMR);
            dbContext.Commit();
            if (documentoMR != null)
            {
