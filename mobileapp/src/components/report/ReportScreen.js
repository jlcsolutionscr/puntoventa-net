...
class ReportScreen extends Component {
  ...
  render () {
    ...
    const { selectedItem, startDate, endDate } = this.state
    const buttonEnabled = selectedItem !== null && startDate != '' && endDate != ''
    return (<View key='1' style={styles.container}>
      ...
      <SearchableDropdown
        ...
        onItemSelect={(item) => this.setState({selectedItem: item})}
      />
      <DatePicker
        label='Fecha inicial'
        value={startDate}
  ...
  handleOnPress () {
    const { selectedItem, startDate, endDate } = this.state
    this.props.generateReportEmail(selectedItem.name, startDate, endDate)
  }
}
