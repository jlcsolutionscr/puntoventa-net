    ...
    const rows = list.map((item, index) => {
      return <View key={index} style={[styles.columnsAlign, styles.contentRow]}>
        <View style={[styles.columnsAlign, {width: '93%'}]}>
          <Text style={{width: '10%', textAlign: 'left', fontSize: (11 * rem)}}>{item.IdFactura}</Text>
          <Text style={{width: '25%', textAlign: 'center', fontSize: (11 * rem)}}>{item.Fecha}</Text>
          <Text style={{width: '65%', textAlign: 'left', fontSize: (11 * rem)}}>{item.NombreCliente}</Text>
          <Text style={{width: '20%', textAlign: 'right', fontSize: (11 * rem)}}>{'IMPUESTO:'}</Text>
          <Text style={{width: '25%', textAlign: 'right', fontSize: (11 * rem)}}>{formatCurrency(item.Impuesto)}</Text>
          <Text style={{width: '20%', textAlign: 'right', fontSize: (11 * rem)}}>{'TOTAL:'}</Text>
          <Text style={{width: '25%', textAlign: 'right', fontSize: (11 * rem)}}>{formatCurrency(item.Total)}</Text>
        </View>
        <View style={{width: '7%'}}><CircleButton size={20 * rem} disabled={item.Estado == 'Anulando'} iconButton={RemoveIcon} primaryColor={'red'} onPressButton={() => this.handleOnPress(item.IdFactura)}/></View>
      </View>
    })
